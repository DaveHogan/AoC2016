...
namespace AdventOfCode2016
{
    /*
        ...
        such as xyyx or abba. However, the IP also must not have an ABBA within any hypernet sequences, which are
    ...
    */
    public class Day7
    {
        ...
        public class Ip7
        {
            public List<string> OuterParts { get; set; }
            public List<string> BracketParts { get; set; }
            public string Raw { get; set; }
            public Ip7()
            {
                OuterParts = new List<string>();
                BracketParts = new List<string>();
            }
        }
        ...
        internal bool ContainsAbba(string toCheck)
        {
            var foundDups = (from d in DupChars where toCheck.Contains(d) select d).ToList();

            if (!foundDups.Any())
                return false;

            foreach (var foundDup in foundDups)
            {
                var indexOfFound = toCheck.IndexOf(foundDup);
                if (indexOfFound == 0 || indexOfFound == toCheck.Length - 2)
                    continue;
                var lChar = toCheck.Substring(indexOfFound - 1, 1);
                var rChar = toCheck.Substring(indexOfFound + 2, 1);

                // Only true if both left and right characters are the same and different to the middle.
                if (lChar == rChar && (lChar != foundDup.First().ToString()))
                {
                    return true;
                }
            }
        ...
        private List<string> _dupChars = null;
        private List<string> DupChars
        {
            get
            {
                if (_dupChars == null)
                {
                    _dupChars = new List<string>();
                    var alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".ToLower().ToCharArray();
                    for (int i = 0; i < alphabet.Length; i++)
                    {
                        _dupChars.Add(alphabet[i].ToString() + alphabet[i]);
                    }
                }
        ...
        {
            var lines = RealInput.Split(Environment.NewLine.ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
            var ip7S = new List<Ip7>();
            foreach (var line in lines)
            {
                ...
                {
                    var left = newLine.IndexOf("[");
                    var right = newLine.IndexOf("]");
                    if (left >= 0 && right > 0)
                    {
                        var bracketBit = newLine.Substring(left+1, right-left-1);
                        var outerBit = newLine.Substring(0, left);

                        newLine = newLine.Substring(right+1);

                        ip7.OuterParts.Add(outerBit);
                        ip7.BracketParts.Add(bracketBit);
                    }
                }
                if (string.IsNullOrWhiteSpace(newLine) == false)
                {
                    ip7.OuterParts.Add(newLine);
                }
            ...
            int cnt = 0;
            foreach (var ip in ip7S)
            {
                var existsInOuter = ip.OuterParts.Any(ContainsAbba);
                var existsInBrackets = ip.BracketParts.Any(ContainsAbba);
                if (existsInOuter && existsInBrackets == false)
                {
                    cnt++;
                }
            }
            ...
            // 87 Too low
            // 85 ? Too low
            //var midChar = found.First();
            //var lIndexOfAlpha = Array.IndexOf(alphabet, found) - 1;
        ...
        public static String DemoInput = @"abba[mnop]qrst
abcd[bddb]xyyx
aaaa[qwer]tyui
ioxxoj[asdfgh]zxcvbn";


        public static String RealInput = @"dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
